...
var topics = []chatroom.Topic{excludeTraitor}
...
var laws = []*regexp.Regexp{
	...
	regexp.MustCompile(`[^!！。、]+が(苦手|にがて)なフレンズなんだね`),
	regexp.MustCompile(`[^!！。、]+が(得意|とくい)なフレンズなんだね`),
...
func excludeTraitor(room chatroom.Room) chatroom.DidTalk {
	r := waitReceived(room)
	if isLegal(r.text) {
		return false
	}
